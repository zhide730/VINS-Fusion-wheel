...
std::mutex m_buf;
custom_msgs::EncoderConstPtr last_encoder_msg;
...
void wheel_callback(const custom_msgs::EncoderConstPtr &encoder_msg)
{
    if (!last_encoder_msg)
    {
        last_encoder_msg = encoder_msg;
        return;
    }
    auto current_encoder_msg = encoder_msg;
    double dt = current_encoder_msg->header.stamp.toSec() - last_encoder_msg->header.stamp.toSec();
...
int main(int argc, char **argv)
{
    ...
    if (USE_WHEEL)
    {
        sub_wheel = n.subscribe(WHEEL_TOPIC, 2000, wheel_callback, ros::TransportHints().tcpNoDelay());
    }
