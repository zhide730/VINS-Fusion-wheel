...
double solveScale(map<double, ImageFrame> &all_image_frame)
{
    ...
    for (frame_i = all_image_frame.begin(); next(frame_i) != all_image_frame.end(); frame_i++)
    {
        ...
        tmp_b = frame_j->second.pre_integration->delta_eta - frame_i->second.pre_integration->delta_q * TIO + TIO + frame_i->second.R.transpose() * frame_j->second.R * TIC[0] - TIC[0];
...
*/
//求解 g, v和s(在solveScale函数中单独求再加到x中)
bool LinearAlignment(map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x)
{
    int all_frame_count = all_image_frame.size();
    // int n_state = all_frame_count * 3 + 3 + 1;
